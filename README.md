...
1. Windowsにおいて、LAN カードが 2 枚以上存在し、一方にデフォルトゲートウェイが設定されており、もう一方に使用するDNSサーバーが設定されているケースで、DNSの名前解決の失敗によるリトライが発生し、接続時に長時間(60秒程度)画面が固まってしまう問題を解決しました。
...
1. **『ビルドにあたっての重要な変更点』 (開発者およびシステム関係者の方々は必ずご一読ください)**
    ...
    - 背景と従来の問題点 - 『IPA-DN-Ultra』 サブリポジトリの極端な肥大化
    ...
    - 解決策 - 『IPA-DN-Ultra』→『IPA-DN-ThinLib』へのリポジトリ名の変更と、古い履歴の削除による肥大化の解消
    ...
    - 『IPA-DN-Ultra』の内容を変更されていた開発者／運用者の方々へのローカルファイルパスの変更のお願い
