...
ICMP_RESULT *IcmpParseResult(IP *dest_ip, USHORT src_id, USHORT src_seqno, UCHAR *recv_buffer, UINT recv_buffer_size, IP *src_ip_v6)
{
	...
	else if (IsIP6(dest_ip))
	{
		...
		if (icmp_packet_size >= sizeof(ICMP_HEADER))
		{
			...
			// if (chksum2 == chksum) // Impossible to calc the ICMPv6 checksum
			                          // because we cannot obtain the destination (local) IPv6 address by RecvFrom
			{
				...
				else
				{
					// If an error is returned, compare to the copy of
					// the ICMP packet last sent
					if (recv_buffer_size >= (4 + sizeof(ICMP_HEADER) + sizeof(IPV6_HEADER)))
					{
						UCHAR *orig_ipv6 = recv_buffer + 4 + sizeof(ICMP_HEADER);
						UINT orig_ipv6_size = recv_buffer_size - 4 - sizeof(ICMP_HEADER);

						IPV6_HEADER_PACKET_INFO v6info = CLEAN;

						if (ParsePacketIPv6Header(&v6info, orig_ipv6, orig_ipv6_size))
						{
							if (v6info.PayloadSize >= sizeof(ICMP_HEADER) + sizeof(ICMP_ECHO))
							{
								ICMP_HEADER *orig_icmp = (ICMP_HEADER *)(v6info.Payload);
								ICMP_ECHO *orig_echo = (ICMP_ECHO *)((UCHAR *)v6info.Payload + sizeof(ICMP_HEADER));

								if (orig_icmp->Type == ICMPV6_TYPE_ECHO_REQUEST && orig_echo->Identifier == Endian16(src_id) && (src_seqno == 0 || orig_echo->SeqNo == Endian16(src_seqno)))
								{
									IP ip;
									...
									ret->Code = icmp->Code;
									ret->Ttl = 127;
									ret->DataSize = v6info.PayloadSize - (sizeof(ICMP_HEADER) + sizeof(ICMP_ECHO));
									ret->Data = Clone((UCHAR *)v6info.Payload + sizeof(ICMP_HEADER) + sizeof(ICMP_ECHO),
										ret->DataSize);
									Copy(&ret->IpAddress, &ip, sizeof(IP));
								}
							}
...
ICMP_RESULT *IcmpEchoSendBySocket_v6Internal(IP *dest_ip, UCHAR ttl, UCHAR *data, UINT size, UINT timeout)
{
	...
	if (s != NULL)
	{
		...
		send_icmp_header->Checksum = 0; // ICMPv6 checksum value is generated by kernel
